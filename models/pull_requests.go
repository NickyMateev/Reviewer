...
var (
	pullRequestColumns               = []string{"id", "user_id", "title", "url", "github_id", "created_at", "updated_at"}
	pullRequestColumnsWithoutDefault = []string{"user_id", "title", "url", "github_id"}
...
func (o *PullRequest) RemoveAuthor(ctx context.Context, exec boil.ContextExecutor, related *User) error {
	var err error
	...
	for i, ri := range related.R.AuthoredPullRequests {
		if queries.Equal(o.UserID, ri.UserID) {
			continue
		}

		ln := len(related.R.AuthoredPullRequests)
		if ln > 1 && i < ln-1 {
			related.R.AuthoredPullRequests[i] = related.R.AuthoredPullRequests[ln-1]
		}
		related.R.AuthoredPullRequests = related.R.AuthoredPullRequests[:ln-1]
		break
	}
	return nil
}
