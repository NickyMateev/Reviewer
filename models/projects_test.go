...
func testProjectToManyContributors(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	_, err = tx.Exec("insert into \"project_users\" (\"project_id\", \"user_id\") values ($1, $2)", a.ID, b.ID)
	if err != nil {
		t.Fatal(err)
	}
	_, err = tx.Exec("insert into \"project_users\" (\"project_id\", \"user_id\") values ($1, $2)", a.ID, c.ID)
	if err != nil {
		t.Fatal(err)
	}

	check, err := a.Contributors().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err = a.L.LoadContributors(ctx, tx, false, (*[]*Project)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Contributors); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.Contributors = nil
	if err = a.L.LoadContributors(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Contributors); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testProjectToManyPullRequests(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := ProjectSlice{&a}
	if err = a.L.LoadPullRequests(ctx, tx, false, (*[]*Project)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testProjectToManyAddOpContributors(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddContributors(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.Contributors[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.Contributors[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.Contributors().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testProjectToManySetOpContributors(t *testing.T) {
	var err error
	...
	err = a.SetContributors(ctx, tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.SetContributors(ctx, tx, true, &d, &e)
	if err != nil {
		t.Fatal(err)
	}
	...
	if a.R.Contributors[0] != &d {
		t.Error("relationship struct slice not set to correct value")
	}
	if a.R.Contributors[1] != &e {
		t.Error("relationship struct slice not set to correct value")
	}
}

func testProjectToManyRemoveOpContributors(t *testing.T) {
	var err error
	...
	err = a.AddContributors(ctx, tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemoveContributors(ctx, tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(a.R.Contributors) != 2 {
		t.Error("should have preserved two relationships")
	}

	// Removal doesn't do a stable deletion for performance so we have to flip the order
	if a.R.Contributors[1] != &d {
		t.Error("relationship to d should have been preserved")
	}
	if a.R.Contributors[0] != &e {
		t.Error("relationship to e should have been preserved")
	}
}

func testProjectToManyAddOpPullRequests(t *testing.T) {
	var err error
...
var (
	projectDBTypes = map[string]string{`ID`: `integer`, `Name`: `character varying`, `RepoName`: `character varying`, `RepoOwner`: `character varying`}
	_              = bytes.MinRead
)
